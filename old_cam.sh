...
libcamera-vid -t 3601s --bitrate 6000000 --autofocus-mode manual --width 1280 --height 720 --framerate 30 --nopreview --output $(date +%Y%m%d_%H%M.mp4) --tuning-file /usr/share/libcamera/ipa/rpi/vc4/imx708.json

# Video Conversion
for video in `ls *.mp4`; do ffmpeg -framerate 30 -i $video -c copy "new_$video"; done
